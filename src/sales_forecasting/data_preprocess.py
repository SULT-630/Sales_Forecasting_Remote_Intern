...
def preprocess_data(df: pd.DataFrame, target_col: str, id_col: str = "record_ID") -> pd.DataFrame:
    ...
    df = encoding_EWMA_features(df, span_values=[2,3,4]) 
    df = encoding_target_changing_rate(df, lag_weeks=[1,2,4]) 
    df = encoding_target_changing_rate_per_gap(df, lag_weeks=[1,2,4]) 
